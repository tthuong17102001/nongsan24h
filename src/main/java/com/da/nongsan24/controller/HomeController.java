...
public class HomeController extends CommomController{
    ...
    //top 20 san pham noi bat
    public void bestRatedProduct20(Model model, User customer) {
        List<Object[]> productList = productRepository.bestRatedProducts20();
        if (productList != null) {
            ArrayList<Integer> listIdProductArrayList = new ArrayList<>();
            for (int i = 0; i < productList.size(); i++) {
                String id = String.valueOf(productList.get(i)[0]);
            ...
            model.addAttribute("bestRatedProduct20", listProductNew);
        }
    }
}
